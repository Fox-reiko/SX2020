...
public class IndexUserPage {
	...
	private JMenuItem s1, s2;// 二级菜单

	public IndexUserPage(String username) {
		indexInit();// 初始化页面
		init(username);// 框架初始化
		action(username);// 确定事件
	...
	private void indexInit() {
		...
		f2.setFont(d);
		// 子菜单
		s1 = new JMenuItem("个人主页");
		s2 = new JMenuItem("确定退出");
		s1.setFont(x);
		s2.setFont(x);
